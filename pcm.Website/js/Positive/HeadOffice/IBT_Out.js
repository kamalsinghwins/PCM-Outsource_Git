...
function searchBranch() {
    _searchType = 'B';
    //$("#chkMaster").css('visibility', 'hidden');
    //$("#chkMasterCode").css('visibility', 'hidden');
...
function getItemCodes() {
    ...
    if (keycode == '13') {
        ...
        if ($('#textBoxItemCode').val() != "") {
            ...
            //if (DefaultPurchaseTaxInt != 0) {
            //    $('#' + _dropDownTaxId).val(DefaultPurchaseTaxInt - 1)
            //}
            //else {
            //    $('#' + _dropDownTaxId).removeAttr('disabled')
            //    var mymodal = $('#messagePopup');
            //    mymodal.find('.modal-body').text('You must set a Default Sales Tax in the Company Settings be before you can continue.');
            //    mymodal.find('.modal-title').text('Error');
            //    mymodal.modal('show');
            //}
            //return;
...
function searchItems() {
    ...
    //$("#chkMasterCode").css('visibility', 'visible');
    var mymodal = $('#Searchbrit');
...
function showItemCodeDetails(generatedCodeInfoResponse) {
    if (generatedCodeInfoResponse.GetGeneratedCodeInfo == true) {
        ...
        //    '================================================================================
        //    'Get the Purchase TAX percentage
...
function clearLineItem() {
    ...
    $("#textBoxDescription").val('')
    $("#txtExcl").val('')
    //$("#" + _dropDownTaxId + " option:selected").text() 
    $("#txtIncl").val('')
    $("#txtQty").val('')
    $("#txtTotalIncl").val('')
    $("#lblCat1").text('')
...
function CalculateTaxAmount() {
    ...
    $("#lblTaxAmount").text(((parseFloat($("#txtExcl").val()) * parseFloat((tmpInTaxValue)) / 100) * parseFloat($("#txtQty").val())).toFixed(2))
    $("#txtTotalIncl").val((parseFloat($("#txtIncl").val()) * parseFloat($("#txtQty").val())).toFixed(2))
}
...
function CalculateTotalCostIncl(element) {
    $(element).closest('tr').find('.totalcostinput').val((parseFloat($(element).closest('tr').find('.sum').text()) * (parseFloat($(element).closest('tr').find('.qtyinput').val()))).toFixed(2))
...
function downloadLabel(path) {
    ...
    //$.post(RequestUrl, { FormData: path }, function (respone) {
    //    if (respone.Success == true) {
