...
Public Class ShopTransactionsDL
    ...
    Public Function ProcessCashTransaction(ByVal CashTransaction As CashTransaction, ByVal ServerPath As String) As String
        ...
        tmpSQL = "INSERT INTO cash_transactions " & _
                 "(guid,sale_date,sale_time,branch_code,user_name,till_number,transaction_type,transaction_number,sale_total,cash,cheque," & _
                 "credit_card,voucher,account,payment_details)" & _
                 " VALUES('" & RG.Apos(CashTransaction.transaction_guid) & "','" & CashTransaction.sale_date & "','" & CashTransaction.sale_time & "','" & Mid$(RG.Apos(CashTransaction.branch_code), 1, 5) & "'," & _
                 "'" & Mid$(RG.Apos(CashTransaction.user_name), 1, 19) & "','" & Mid$(RG.Apos(CashTransaction.till_number), 1, 3) & "','" & Mid$(RG.Apos(CashTransaction.transaction_type), 1, 20) & "'," & _
                 "'" & Mid$(RG.Apos(CashTransaction.transaction_number), 1, 15) & "'," & _
                 "'" & RG.Apos(RG.Numb(CashTransaction.sale_total)) & "','" & RG.Apos(RG.Numb(CashTransaction.cash)) & "','" & RG.Apos(RG.Numb(CashTransaction.cheque)) & "'," & _
                 "'" & RG.Apos(RG.Numb(CashTransaction.credit_card)) & "'," & _
    ...
    Public Function ProcessHeader(ByVal TransactionData As TransactionMaster, ByVal ServerPath As String) As String
        ...
        tmpSQL = "INSERT INTO transaction_times (guid,period,currrent_hour,current_weekday,current_week) VALUES " & _
                "('" & RG.Apos(TransactionData.transaction_guid) & "','1" & TransactionData.current_month & "','" & TransactionData.current_hour & "','" & TransactionData.current_weekday & "'," & _
    ...
    Public Sub AddStock(ByVal generated_code As String, _
    ...
    Public Function UpdateStationaryCount(ByVal TransactionData As StationaryTransactionData, ByVal ServerPath As String) As String
        ...
        tmpSQL = "INSERT INTO stationary_transactions (item_code,quantity,transaction_type,branch_code) VALUES " & _
    ...
    Public Function UpdateIBTTransactions(ByVal TransactionData As IBTTransactionData, ByVal ServerPath As String) As String
        ...
        tmpSQL = "UPDATE ibt_transactions SET received_qty = '" & TransactionData.received_qty & "',receiving_date = '" & TransactionData.receiving_date & "' " &
                 ...
                 "AND generated_code = '" & TransactionData.generated_code & "' AND sending_branch_code = '" & TransactionData.sending_branch_code & "'"
        Try
    ...
    Public Function ProcessLineItems(ByVal TransactionData As TransactionLineItems, ByVal ServerPath As String) As String
        ...
        If Val(TransactionData.updated_cost) <> 0 Then 'This should never be run at the store version. Using the same Module as per HO
            If Val(TransactionData.updated_cost) <> Val(TransactionData.cost_exclusive_average) Then
                ...
                tmpSQL = "UPDATE stockcodes_prices SET cost_price = '" & RG.Numb(NewCost) & "',estimated_cost = '" & TransactionData.updated_cost & "'" & _
