...
Public Class ManageUserDataLayer
    ...
    Public Function DeleteUser(deleteRequest As DeleteRequest, BranchCode() As String) As DataTableResponse
        ...
        Try
            tmpSQL = "DELETE FROM user_permissions WHERE branch_code = '" & RG.Apos(BranchCode(0).ToUpper) & "' " &
                     "AND user_name = '" & RG.Apos(deleteRequest.User.ToUpper) & "'"
    ...
    Public Function SaveUser(saveUserRequest As SaveUserRequest, BranchCode() As String, strPOS As String, strMaint As String, strTrans As String, strOther As String) As DataTableResponse
        ...
        Try
            tmpSQL = "SELECT * FROM user_permissions WHERE user_name = '" & RG.Apos(saveUserRequest.User.ToUpper) & "' " &
                     "AND branch_code <> '" & BranchCode(0).ToUpper & "'"
            ds = objDB.GetDataSet(tmpSQL)
            If objDB.isR(ds) Then
                dataTableResponse.Detail = ds.Tables(0).Rows(ds.Tables(0).Rows.Count - 1)("user_name")
                dataTableResponse.Success = False
                dataTableResponse.Message = "User: " & saveUserRequest.User.ToUpper & " already exists on the system for another store. Please select another username."
                Return dataTableResponse
            End If
            ...
            tmpSQL = "SELECT user_name FROM user_permissions WHERE branch_code = '" & RG.Apos(BranchCode(0).ToUpper) & "' " &
                     "AND user_name = '" & RG.Apos(saveUserRequest.User) & "'"
            ds = objDB.GetDataSet(tmpSQL)
            If objDB.isR(ds) Then
                ds.Clear()
                tmpSQL = "UPDATE user_permissions SET user_password = '" & RG.Apos(saveUserRequest.Password.ToUpper) & "'," &
                         ...
                         " WHERE branch_code = '" & RG.Apos(BranchCode(0).ToUpper) & "' AND " &
                         "user_name = '" & RG.Apos(saveUserRequest.User.ToUpper) & "'"
                objDB.ExecuteQuery(tmpSQL)
            Else
