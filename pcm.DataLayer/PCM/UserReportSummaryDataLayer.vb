...
Public Class UserReportSummaryDataLayer
    ...
    Public Function ReturnUsersSummaryTransactions(ByVal StartDate As String, ByVal EndDate As String) As DataTable
        ...
        Try
            ...
            Dim strQuery As String = "SELECT " & _
                                     ...
                                     "TO_CHAR(timestamp_of_action, 'YYYY-MM-DD HH24:MI:SS') AS timestampofaction," & _
                                     "account_number AS accountnumber,user_comment AS usercomment," & _
                                     "action_result AS actionresult," & _
                                     "TO_CHAR(ptp_date, 'YYYY-MM-DD HH24:MI:SS') AS ptpdate," & _
                                     "ptp_amount AS ptpamount," & _
                                     "TO_CHAR((length_of_action || ' second')::interval, 'HH24:MI:SS') AS lengthofaction," & _
                                     "collections_period AS collectionsperiod " & _
                                     "FROM users_actions " & _
                                     "WHERE timestamp_of_action between '" & StartDate & " 00:00:00' and '" & EndDate & " 23:59:59' " & _
                                     "AND action_type <> 'User Login' " & _
    ...
    Public Function AccountsOpenedByEmployee(ByVal StartDate As String, ByVal EndDate As String) As DataTable
        ...
        Try
            ...
            command.Connection = connection
            command.CommandType = CommandType.Text
            Dim strQuery As String = "SELECT TRIM(LOWER(employee_number)) AS employee_number," & _
                                     "COUNT (dd.account_number) as total_opened," & _
                                     "COUNT (CASE WHEN status = 'ACTIVE' THEN status END) as total_active," & _
                                     "COUNT (CASE WHEN total_spent > 50 THEN total_spent END) AS total_spent " & _
                                     "FROM debtor_dates dd " & _
                                     ...
                                     "AND (employee_number IS NOT NULL AND employee_number <> '') " & _
                                     "GROUP BY TRIM(LOWER(employee_number)) " & _
